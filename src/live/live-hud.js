...
function ballEffect(p){const type=text(p.ballType,'tier1'),config=GAME_RULES.types[type]||GAME_RULES.types.tier1,q=Math.max(1,Number(p.quantity)||1);return{value:q===1?`${config.damage}`:`${q}× · ${config.damage}`,label:q===1?'DANO':'DANO CADA'}}
function effect(rule){const p=params(rule);if(ruleAction(rule)==='drop_ball')return ballEffect(p);if(ruleAction(rule)==='restart')return{value:'REINICIAR',label:'PARTIDA'};return{value:ruleAction(rule)||'AÇÃO',label:''}}
export function createLiveHud(){
 ...
 function render(){if(!root||!list)return;list.replaceChildren();if(!rules.length){const empty=document.createElement('div');empty.className='live-rule-empty';empty.textContent='Aguardando regras do painel';list.appendChild(empty);syncLayout();return}for(const rule of rules){const card=document.createElement('article');card.className='live-rule-card';const icon=document.createElement('div');icon.className='live-rule-icon';const src=giftIcon(rule);if(src){const img=document.createElement('img');img.src=src;img.alt='';img.referrerPolicy='no-referrer';icon.appendChild(img)}else icon.textContent='🎁';const copy=document.createElement('div');copy.className='live-rule-copy';const b=document.createElement('b');b.textContent=giftName(rule);const e=effect(rule);const value=document.createElement('strong');value.className='live-rule-value';value.textContent=e.value;const label=document.createElement('small');label.className='live-rule-label';label.textContent=e.label;copy.append(b,value,label);card.append(icon,copy);list.appendChild(card)}syncLayout()}
