...
export function createLiveHud(){
 ...
 function syncLayout(){if(!slots)return;if(!root||root.hidden){slots.style.bottom='max(18px, env(safe-area-inset-bottom))';return}const hudRect=root.getBoundingClientRect(),stageRect=stage?.getBoundingClientRect()||{bottom:window.innerHeight};const bottom=Math.max(0,stageRect.bottom-hudRect.top+LAYOUT_GAP);slots.style.bottom=`${Math.ceil(bottom)}px`}
 ...
 window.addEventListener('resize',syncLayout,{passive:true});window.addEventListener('orientationchange',syncLayout,{passive:true});window.addEventListener('pageshow',()=>{setVisible(savedVisible(),{persist:false});syncLayout()});setVisible(savedVisible(),{persist:false});render();
