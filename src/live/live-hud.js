...
export function createLiveHud(){
 ...
 function syncLayout(){if(!slots)return;if(!root||root.hidden){slots.style.bottom='max(18px, env(safe-area-inset-bottom))';return}const rect=root.getBoundingClientRect();const bottom=Math.max(0,window.innerHeight-rect.top+LAYOUT_GAP);slots.style.bottom=`${Math.ceil(bottom)}px`}
