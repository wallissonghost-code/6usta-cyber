...
test('5 bolas entram fisicamente nos slots e contabilizam uma unica vez',async({page})=>{
 await page.evaluate(()=>{CyberGame.reset();for(let i=0;i<5;i++)CyberGame.triggerLike('FiveBall')});
 ...
 expect(result.state.topUser).toBe('FiveBall');expect(result.state.topScore).toBe(slots.reduce((sum,x)=>sum+x.points,0));
});
test('bots simulam partida completa sem acumular bolas',async({page})=>{
 await page.evaluate(()=>{CyberGame.reset();for(let i=0;i<36;i++){setTimeout(()=>CyberGame.triggerLike('BotLike'+i),i*45);if(i%4===0)setTimeout(()=>CyberGame.triggerComment('BotChat'+i,'!drop'),i*45+15);if(i%9===0)setTimeout(()=>CyberGame.triggerGift('BotGift'+i,i%18===0?'Capivara':'Rosa'),i*45+25)}});
 await page.waitForTimeout(2500);await expect.poll(async()=>(await state(page)).activeBalls,{timeout:20000,intervals:[500,1000]}).toBeLessThanOrEqual(2);
 const s=await state(page);expect(s.topScore).toBeGreaterThan(0);expect(await page.evaluate(()=>CyberGame.getBallAudit().filter(x=>x.event==='slot').length)).toBeGreaterThan(0);
});
test('rajada de presentes termina e runtime continua responsivo',async({page})=>{
 await page.evaluate(()=>{CyberGame.reset();for(let i=0;i<10;i++)CyberGame.triggerGift('Stress'+i,i%3===0?'Galáxia':i%3===1?'Capivara':'Rosa')});
 await page.waitForTimeout(1000);await expect.poll(async()=>(await state(page)).activeBalls,{timeout:25000,intervals:[500,1000]}).toBeLessThanOrEqual(2);
...
test('resize durante partida não deixa corpos órfãos',async({page})=>{
 await page.evaluate(()=>{CyberGame.reset();for(let i=0;i<12;i++)CyberGame.triggerLike('ResizeBot'+i)});await page.setViewportSize({width:390,height:700});await page.evaluate(()=>CyberGame.resize());await page.waitForTimeout(1000);
